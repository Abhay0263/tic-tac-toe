...
function updateLeaderboard() {
  fetch("http://13.201.189.45:5000/leaderboard")
    .then(res => res.json())
    .then(data => {
...
function openModal(mode) {
  ...
  startMatchBtn.onclick = async () => {
    ...
    // Check DB
    try {
      let res = await fetch(`http://13.201.189.45:5000/check_username/${p1}`);
      if (res.ok) {
      ...
      if (!botMode) {
        res = await fetch(`http://13.201.189.45:5000/check_username/${p2}`);
        if (res.ok) {
          let data = await res.json();
          if (data.exists) {
...
async function startMatchOnServer() {
  try {
    const res = await fetch("http://13.201.189.45:5000/start", {
      method: "POST",
...
async function makeMove(pos) {
  try {
    const res = await fetch("http://13.201.189.45:5000/move", {
      method: "POST",
      headers: {"Content-Type": "application/json"},
